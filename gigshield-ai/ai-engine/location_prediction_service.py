"""
Runtime helpers for next-destination prediction and mismatch detection.
"""
...
MODEL_ENV_VAR = "NEXT_LOCATION_MODEL_PATH"
REFERENCE_ENV_VAR = "NEXT_LOCATION_REFERENCE_PATH"


def _candidate_paths() -> Tuple[List[Path], List[Path]]:
    current_dir = Path(__file__).resolve().parent
    configured_model_path = os.getenv(MODEL_ENV_VAR, "").strip()
    configured_reference_path = os.getenv(REFERENCE_ENV_VAR, "").strip()

    model_paths = [
        Path(configured_model_path) if configured_model_path else None,
        current_dir / "next_location_model.pkl",
        current_dir / "models" / "next_location_model.pkl",
        Path(r"E:\Downloads\next_location_model.pkl"),
    ]
    reference_paths = [
        Path(configured_reference_path) if configured_reference_path else None,
        current_dir / "data" / "next_location_reference.csv",
    ]
...
def _resolve_existing_path(paths: List[Path], label: str) -> Path:
    for path in paths:
        if path.exists():
            return path
...
def load_location_model():
    ...
    model_path = _resolve_existing_path(model_paths, "next_location_model.pkl")
...
def predict_location(payload: object) -> Dict[str, object]:
    ...
    features = pd.DataFrame([[values[column] for column in LOCATION_FEATURE_COLUMNS]], columns=LOCATION_FEATURE_COLUMNS)
...
def decode_destination(encoded_destination_id: int, reference_df: pd.DataFrame) -> Dict[str, object]:
    match = reference_df.loc[
        reference_df["encoded_destination_id"] == int(encoded_destination_id)
    ]
    if match.empty:
        return {
