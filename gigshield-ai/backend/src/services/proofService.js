...
async function resolveClaimContext({
  ...
  const normalizedClaimId = normalizeString(claimId, `claim-${Date.now()}`);
  const normalizedUserId = normalizeString(userId, "session-worker");
  ...
  if (!isUuid(userId)) {
    return {
      ...
      reason: fallbackInactive
        ...
        : "Fallback activity heuristic assumes the worker was active.",
    };
  }
...
async function calculateAnomalyScore({ userId, claimContext }) {
  if (!isUuid(userId)) {
    return {
      anomaly_score: 12,
      reasons: ["Using fallback anomaly scoring because no persisted worker record exists."],
    };
  }
